...
const Navbar = ({ onOpenDonate }) => {
  ...
  const fetchLiveImpact = async () => {
    try {
      const [volRes, missionRes] = await Promise.all([
        ...
        fetch(getEndpoint('/api/missions'))
      ]);
  ...
  useEffect(() => {
    ...
    const tickerTimer = setInterval(() => {
      setLiveStats((currentStats) => {
        setTickerIndex((prev) => (prev + 1) % currentStats.length);
        return currentStats;
      });
    }, 3000);
  ...
  const navLinks = [
    ...
    { name: 'Explore Us', href: '/explore' },
    { name: 'Our Team', href: '/team' }
    
  ];

  return (
    <nav className={`fixed top-0 w-full z-[100] transition-all duration-500 px-6 py-4 bg-white/90 backdrop-blur-xl border-b border-slate-100/80 shadow-sm ${
      scrolled ? 'shadow-md' : ''
    }`}>
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <div className="hidden lg:flex items-center gap-10">
          ...
          <button 
            ...
            className="relative bg-[#0a1f44] text-white w-44 h-11 rounded-full font-bold text-[10px] uppercase tracking-[0.15em] shadow-lg overflow-hidden flex items-center justify-center hover:bg-[#0052AD] transition-all"
          >
